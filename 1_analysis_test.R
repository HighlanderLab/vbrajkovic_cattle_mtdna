...
setwd("/Users/vladimir/Desktop/Testiranje_koda_Github_mitogenome/ipocrnic_cattle_mtdna-main/Data")

#################### The order of the packages is as they are needed in the code

library(package = "tidyverse")
library(package = "pedigreemm")
library(package = "nadiv")
library(package = "terra") #################### install.packages('terra', repos='https://rspatial.r-universe.dev') #"raster" package didn't work without "terra" package
library(package = "raster") #################### Warning message: In getData("GADM", country = "HRV", level = 1) :getData will be removed in a future version of raster. You can use access these data, and more, with functions from the geodata package instead.
library(package = "maptools") #################### Please note that 'maptools' will be retired by the end of 2023, plan transition at your earliest convenience; some functionality will be moved to 'sp'.
...
source("/Users/vladimir/Desktop/Testiranje_koda_Github_mitogenome/ipocrnic_cattle_mtdna-main/functions.R")
...
Phe = read_csv(file = "koor_feno_merge.csv", col_names = TRUE,
...
Snp = read_table(file = "arle.hap", col_names = FALSE)
# head(Snp)
...
# Add map of Croatia:
mapaHR = getData('GADM', country="HRV", level= 1)  ################### raster package needed


#####   TABLE 1 - paper  #######

# Extract the border from the map
hr.border <- unionSpatialPolygons(mapaHR, rep(1, nrow(mapaHR))) ################### maptools package needed
...
# plot(mesh3, main="3rd mesh"); points(koordinate$long, koordinate$lat, col = "red") ################### working
...
# dim(A3) #################### 7115 772 - not the same as 7115 771
# 7115 771
...
SummarizeInlaVars(FitMilkLact15ZSstack)
 ...
 write.csv(SummarizeInlaVars(FitMilkLact15ZSstack), file = "Inla_var_FitMilkLact15ZSstack.csv")
...
sol_m = s.eff[,97:458]
# Calculate variance:
mean(matrixStats::rowVars(sol_m)) #################### 0.02010276
# Equal to var from hyperparam: 
mean(1 / inla.rmarginal(n = 1000, marginal = FitMilkLact15ZSstack$marginals.hyperpar[["Precision for snpovi"]])) #################### 0.02105081

# For the haplotype solutions:
sol_haps = s.eff[,1:96]
# Calculate Var(hap)
mean(matrixStats::rowVars(sol_haps)) #################### 0.08295023
# The haplotype solutions have to be from sampling

# Residual has to go from the hyperparameter:
mean(1 / inla.rmarginal(n = 1000, marginal = FitMilkLact15ZSstack$marginals.hyperpar[["Precision for the Gaussian observations"]])) #################### 0.3515865
mean(unlist(lapply(sample1, function(x) 1/(x$hyperpar["Precision for the Gaussian observations"])))) #################### 0.3515598
...
mean(unlist(TotGenetic)) #################### 0.08294883
...
write.csv(round(LDb_P,2),"LDb_MLP.csv", row.names = T)
...
round(mean(unlist(lapply(OverSamples, function(x) x$Ve))), 4) #################### 0.3516
round(mean(unlist(lapply(OverSamples, function(x) x$Va))), 4) #################### 0.2985
round(mean(unlist(lapply(OverSamples, function(x) x$Vs))), 4) #################### 0.0145
round(mean(unlist(lapply(OverSamples, function(x) x$Vp))), 4) #################### 0.0899
round(mean(unlist(lapply(OverSamples, function(x) x$Vhys))), 4) #################### 0.0989
round(mean(unlist(lapply(OverSamples, function(x) x$Vhap))), 4) #################### 0.083
round(mean(unlist(lapply(OverSamples, function(x) x$Vloc))), 4) #################### 0.0459

round(sd(unlist(lapply(OverSamples, function(x) x$Ve))), 4) #################### 0.0056
round(sd(unlist(lapply(OverSamples, function(x) x$Va))), 4) #################### 0.0202
round(sd(unlist(lapply(OverSamples, function(x) x$Vs))), 4) #################### 0.0078
round(sd(unlist(lapply(OverSamples, function(x) x$Vp))), 4) #################### 0.0143
round(sd(unlist(lapply(OverSamples, function(x) x$Vhys))), 4) #################### 0.0055
round(sd(unlist(lapply(OverSamples, function(x) x$Vhap))), 4) #################### 0.0119
round(sd(unlist(lapply(OverSamples, function(x) x$Vloc))), 4) #################### 0.0062


# Ratios:
round(mean(unlist(lapply(OverSamples, function(x) x$r2))), 4) #################### 0.3581
round(mean(unlist(lapply(OverSamples, function(x) x$h2))), 4) #################### 0.3038
round(mean(unlist(lapply(OverSamples, function(x) x$s2))), 4) #################### 0.0147
round(mean(unlist(lapply(OverSamples, function(x) x$p2))), 4) #################### 0.0916
round(mean(unlist(lapply(OverSamples, function(x) x$hys2))), 4) #################### 0.1008
round(mean(unlist(lapply(OverSamples, function(x) x$m2))), 4) #################### 0.0844
round(mean(unlist(lapply(OverSamples, function(x) x$loc2))), 4) #################### 0.0467

round(sd(unlist(lapply(OverSamples, function(x) x$r2))), 4) #################### 0.0087
round(sd(unlist(lapply(OverSamples, function(x) x$h2))), 4) #################### 0.018
round(sd(unlist(lapply(OverSamples, function(x) x$s2))), 4) #################### 0.0079
round(sd(unlist(lapply(OverSamples, function(x) x$p2))), 4) #################### 0.0148
round(sd(unlist(lapply(OverSamples, function(x) x$hys2))), 4) #################### 0.0058
round(sd(unlist(lapply(OverSamples, function(x) x$m2))), 4) #################### 0.0113
round(sd(unlist(lapply(OverSamples, function(x) x$loc2))), 4) #################### 0.006
